...
function seeFile(fileName) {
    window.open("/get/" + fileName + "?rules=" + getRules() + "&layout=" + getLayout(), '_blank').focus();
}
function seeAll() {
    window.open("/get/all" + "?rules=" + getRules() + "&layout=" + getLayout(), '_blank').focus();
}
function previewFile(fileName) {
    window.open("/preview.html?filename=" + fileName + "&layout=" + getLayout(), '_blank').focus();
}
...
function getLayout() {
    return [...document.querySelectorAll('#rules input[type=text]')].map(e => e.value).join("_");
}
